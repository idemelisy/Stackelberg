...
namespace test_framework {
    ...
    bool ParserTest::test_tiger_problem_parsing() {
        ...
        try {
            ...
            assert_true(problem.discount_factor > 0.0 && problem.discount_factor <= 1.0, 
                       "Tiger discount factor should be in (0,1]");
            assert_false(problem.states.empty(), "Tiger problem should have states");
            assert_false(problem.actions.empty(), "Tiger problem should have actions");
            assert_false(problem.observations.empty(), "Tiger problem should have observations");
